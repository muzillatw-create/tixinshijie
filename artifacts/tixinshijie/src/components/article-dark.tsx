...
export interface ArticleDarkData {
  ...
  seriesImage?: string;
  seriesImage2?: string;
  seriesImage2Title?: string;
  seriesImage2Text?: string;
  seriesImage3?: string;
  ...
  seriesImage5?: string;
  seriesImage5Title?: string;
  seriesImage5Text?: string;
...
export function ArticleDark({ data }: { data: ArticleDarkData }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-20 overflow-x-hidden">
      ...
      {/* Hero Image */}
      <div className="rounded-2xl overflow-hidden mb-8 shadow-xl shadow-black/40">
        <img src={data.heroSrc} alt={data.heroAlt} className="block w-full h-auto" loading="lazy" />
      </div>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_240px] gap-10 items-start">
        ...
        <article className="min-w-0 overflow-hidden [&_img]:max-w-full [&_img]:block">
            ...
            return (
              <section key={section.id} id={section.id} className="mb-10 scroll-mt-24 overflow-hidden">
                ...
                {showImg && (() => {
                  ...
                  return imgUrl
                    ? (
                      <div className="overflow-hidden rounded-2xl my-6 shadow-lg">
                        <img src={imgUrl} alt={`${data.category}精彩圖片 ${currentImgIdx + 1}`} className="block w-full h-auto" loading="lazy" />
                      </div>
                    )
          ...
          {data.seriesImage2 && (
            <div className="my-8">
              {(data.seriesImage2Title || data.seriesImage2Text) && (
                <div className="mb-4">
                  {data.seriesImage2Title && (
                    <h3 className="text-xl font-bold text-cyan-400 mb-2">{data.seriesImage2Title}</h3>
                  )}
                  {data.seriesImage2Text && (
                    <p className="text-gray-400 leading-relaxed">{data.seriesImage2Text}</p>
                  )}
                </div>
              )}
          ...
          {data.seriesImage3 && (
            <div className="my-8">
              {(data.seriesImage3Title || data.seriesImage3Text) && (
                <div className="mb-4">
                  {data.seriesImage3Title && (
                    <h3 className="text-xl font-bold text-cyan-400 mb-2">{data.seriesImage3Title}</h3>
                  )}
                  {data.seriesImage3Text && (
                    <p className="text-gray-400 leading-relaxed">{data.seriesImage3Text}</p>
                  )}
                </div>
              )}
              <div className="overflow-hidden rounded-2xl shadow-lg">
                ...
                {dbArticles.map(a => (
                  <div key={a.id} className="min-w-0 bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-cyan-500/40 transition-all hover:-translate-y-0.5 group">
                    <div className="overflow-hidden">
                      <img src={a.heroImage || undefined} alt={a.title} className="block w-full h-auto group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                    </div>
                    <div className="p-4">
